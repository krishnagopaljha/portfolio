...
function startMatrixRain() {
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    function executeCommand(command) {
      ...
      const durationInSeconds = parseInt(commandParts[2], 10);
      ...
      switch (mainCommand) {
        ...
        case 'clear':
          ...
          break;
        case 'magic':
          if (option === '-t' && !isNaN(durationInSeconds)) {
            startMatrixRain();
            setTimeout(() => {
              stopMatrixRain();
            }, durationInMilliseconds); // Use the specified duration in milliseconds
          } else {
            startMatrixRain();
            setTimeout(() => {
              stopMatrixRain();
